...
export async function deployAAFactory(wallet: Wallet): Promise<Contract> {
    ...
    const codeHash = utils.hashBytecode(accountArtifact.bytecode);
    return await deployer.deploy(factoryArtifact, [codeHash], undefined, [accountArtifact.bytecode])
}
...
export async function deployAccount(wallet: Wallet, owner: Wallet, factory_address: string): Promise<Contract> {
    ...
    const account_address = utils.create2Address(factory.address, await factory.codeHash(), salt, AbiCoder.encode(["address"], [owner.address]))
    const accountArtifact = await deployer.loadArtifact("TestAccount")
